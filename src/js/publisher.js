class Publisher {
    ...
    draw() {
        ...
        //document.getElementById("token").value = "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";
        ...
        function validate() {
            ...
            if (document.getElementById("repo").value === "") {
                ...
                if (document.getElementById("repo").value.indexOf('https://github.com/') > -1) {
                    let strElements = document.getElementById("repo").value.replace('//', '');
                    const elements = strElements.split('/');
                    owner = elements[1];
                    repoName = elements[2];
                } else {
                    html = "Not a valid Git URL!<br/>";
                    errMsg.innerHTML = html;
                    submit = false;
                }
            }
            ...
            if (submit) {
                ...
                send_file(file, queryName, key, owner, repoName);
            }

            async function send_file(file, queryName, key, owner, repo) {
                document.inst(key);
                const commits = await document.client.rest.repos.listCommits({
                    owner: owner,
                    repo: repo,
                });
                ...
                } = await document.client.rest.git.createTree({
                    owner: owner,
                    repo: repo,
                ...
                } = await document.client.rest.git.createCommit({
                    owner: owner,
                    repo: repo,
                    tree: currentTreeSHA,
                    message: `Updated with HuC SPARQL Publisher`,
                ...
                await document.client.rest.git.updateRef({
                    owner: owner,
                    repo: repo,
                    sha: newCommitSHA,
                    ref: "heads/main", // Whatever branch you want to push to
                });
                document.getElementById("mdForm").innerHTML = `Query committed. Available at <a href=\"https://grlc.io/api-git/${owner}/${repo}\">grlc</a>`;
                //console.log(commits);
